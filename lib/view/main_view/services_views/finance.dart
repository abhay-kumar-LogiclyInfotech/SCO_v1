...
class _FinanceViewState extends State<FinanceView> with MediaQueryMixin {
  ...
  Future _initializeData() async {
    WidgetsBinding.instance.addPostFrameCallback((callback) async {
      ...
      await Provider.of<MyFinanceStatusViewModel>(context, listen: false)
          .myFinanceStatus();
    });
  }
  ...
  Widget _buildUi(localization) {
    ...
    return Consumer<MyFinanceStatusViewModel>(builder: (context, provider, _) {
  ...
  Widget _salaryDetails(
      {required MyFinanceStatusViewModel provider,
      required LanguageChangeViewModel langProvider,
      required AppLocalizations localization}) {
    final listOfSalaries =
        provider.apiResponse.data?.data?.listSalaryDetials ?? [];
        ...
        expandedContent: financeCard(
            content: [
              CustomInformationContainerField(
                  title: localization.bankName,
                  description: topSalaryDetails?.bankName.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.amount,
                  description: topSalaryDetails?.amount.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.currency,
                  description: topSalaryDetails?.currency.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.theMonth,
                  description:
                      topSalaryDetails?.salaryMonth.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.status,
                  description: topSalaryDetails?.status.toString() ?? '- -',
                  isLastItem: true),
            ],
            localization: localization,
            langProvider: langProvider,
  ...
  Widget _deductionDetails(
      {required MyFinanceStatusViewModel provider,
      required LanguageChangeViewModel langProvider,
      required AppLocalizations localization}) {
    final listOfDeduction =
        provider.apiResponse.data?.data?.listDeduction ?? [];
    ...
    return CustomInformationContainer(
        ...
        expandedContent: financeCard(
            content: [
              CustomInformationContainerField(
                  title: localization.totalDeduction,
                  description:
                      topDeduction?.totalDeduction.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.totalDeducted,
                  description: topDeduction?.totalDeducted.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.deductionPending,
                  description:
                      topDeduction?.deductionPending.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.currency,
                  description: topDeduction?.currencyCode.toString() ?? '- -',
                  isLastItem: true),
            ],
            onTapViewAll: () {
              _navigationServices.pushCupertino(CupertinoPageRoute(
                  builder: (context) => const DeductionDetailsView()));
            },
            langProvider: langProvider,
            localization: localization));
  }
  ...
  Widget _bonusDetails(
    ...
    return CustomInformationContainer(
        ...
        expandedContent: financeCard(
            content: [
              CustomInformationContainerField(
                  title: localization.period,
                  description: topBonus?.periodIdDescription.toString() ??
                      '- -' ??
                      '- -'),
              CustomInformationContainerField(
                  title: localization.term,
                  description: topBonus?.termDescription.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.amount,
                  description: topBonus?.amount.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.currency,
                  description: topBonus?.currencyCode.toString() ?? '- -',
                  isLastItem: true),
            ],
            onTapViewAll: () {
              _navigationServices.pushCupertino(CupertinoPageRoute(
                  builder: (context) => const BonusDetailsView()));
            },
            langProvider: langProvider,
            localization: localization));
  }
    ...
    return CustomInformationContainer(
        ...
        expandedContent: financeCard(
            content: [
              CustomInformationContainerField(
                  title: localization.term,
                  description: topWarning?.termDescription.toString() ?? '- -'),
              CustomInformationContainerField(
                  title: localization.certificateDescription,
                  description:
                      topWarning?.warningCertificate.toString() ?? '- -',
                  isLastItem: true),
            ],
            onTapViewAll: () {
              _navigationServices.pushCupertino(CupertinoPageRoute(
                  builder: (context) => const WarningDetailsView()));
            },
            langProvider: langProvider,
            localization: localization));
  }
